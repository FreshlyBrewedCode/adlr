  ...
  switch (action.type) {
    ...
    case "event": {
      ...
      if (ev.type === "span.started") {
        const d = ev.data as { span_id: string; kind: string; name: string; parent_id?: string | null }
        const newSpan: Span = {
          id: d.span_id,
      ...
      if (ev.type === "span.finished" || ev.type === "span.failed") {
        ...
        return {
          ...
          spans: state.spans.map(s =>
            s.id === d.span_id
              ? { ...s, status: newStatus as Span["status"], finished_at: ev.timestamp }
              : s,
          ),
          events: [ev, ...state.events],
        }
      ...
      if (ev.type === "context.added") {
        const item = ev.data as ContextItem
        return {
          ...state,
