...
import type { ContentNode, LayoutProps } from "../../core/types";
...
function getPanelTitle(node: ContentNode): string {
	...
	if ("panel" in node) {
		return (
			PanelRegistry.get((node as { panel: string }).panel)?.title ??
			(node as { panel: string }).panel
		);
	}
	return "?";
}
...
export function TabsLayout({
	...
	return (
		<box style={{ flexDirection: "column", width, height }}>
				...
				<box style={{ height: 1, flexDirection: "row" }}>
					{childArray.map((_, i) => {
						const title = childNodes
							? getPanelTitle(childNodes[i])
							: String(i + 1);
						const isActive = i === activeIndex;
						return (
							<box key={String(i)} style={{ marginRight: 2 }}>
			...
			<box style={{ flexGrow: 1 }}>{childArray[activeIndex]}</box>
			{tabPosition === "bottom" && (
				<box style={{ height: 1, flexDirection: "row" }}>
					{childArray.map((_, i) => {
						const title = childNodes
