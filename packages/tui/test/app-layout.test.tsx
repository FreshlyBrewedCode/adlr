...
describe("App with custom layout", () => {
  beforeEach(() => {
    PanelRegistry.clear()
  ...
  test("uses custom layout when provided", () => {
    const customLayout = {
      layout: "split" as const,
      ratio: 0.6,
      content: ["overview", "logs"]
    }
    const { lastFrame } = render(<App sessionId="test-123" layout={customLayout} />)
    const frame = lastFrame()
    // In a split layout, both panels should be visible (or at least the layout renders)
    expect(frame).toContain("adler")
    expect(frame).toContain("Overview")
    // Logs panel has a specific header (truncated in narrow split)
    expect(frame).toContain("View")
  })

  test("uses default layout when no layout prop is provided", () => {
