...
export class ProcessManager {
	...
	async spawnAgent(data: {
		...
		const runCmd = agentDef.run?.({
			prompt: data.prompt,
			subagent: data.agentType.split(":")[1],
		});
		if (!runCmd) {
	...
	private async pollStatus(spanId: string) {
		...
		if (result === "completed" || result === "failed" || result === "blocked") {
			await this.completeAgent(
				spanId,
				result === "completed" ? 0 : result === "failed" ? 1 : 0,
				result as SpanStatus,
			);
		}
	}
	...
	private async completeAgent(
		...
		if (agentDef?.output) {
			try {
				...
				this.logger?.error(
					"Agent output hook failed",
					{ agent: String(span.data.agent_type), error },
					{ session_id: span.session_id, span_id: spanId },
				);
			}
		}

		const status: SpanStatus =
			forcedStatus ?? (exitCode === 0 ? "done" : "failed");
		...
		if (status === "done") {
			this.logger?.info(
				"Agent completed",
				{ agent: String(span.data.agent_type), exit_code: exitCode },
				{ session_id: span.session_id, span_id: spanId },
			);
			...
			this.logger?.info(
				"Agent blocked",
				{ agent: String(span.data.agent_type), exit_code: exitCode, status },
				{ session_id: span.session_id, span_id: spanId },
			);
		}
