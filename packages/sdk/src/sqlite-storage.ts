...
export class SQLiteStorage implements Storage {
	...
	updateSpan(id: string, data: Partial<Span>): Promise<void> {
		const existing = this.db
			.query("SELECT * FROM spans WHERE id = ?")
			.get(id) as Record<string, unknown> | null;
	...
	listSpans(sessionId: string): Promise<Span[]> {
		const rows = this.db
			.query("SELECT * FROM spans WHERE session_id = ? ORDER BY started_at ASC")
	...
	listAllSpans(): Promise<Span[]> {
		const rows = this.db
			.query("SELECT * FROM spans ORDER BY started_at ASC")
			.all() as Record<string, unknown>[];
		return Promise.resolve(
			rows.map((r) => ({
				id: String(r.id),
	...
	listEvents(sessionId: string, filter?: EventFilter): Promise<Event[]> {
		...
		if (filter?.span_id !== undefined) {
			conditions.push("span_id = ?");
			values.push(filter.span_id);
		}
		const sql = `SELECT * FROM events WHERE ${conditions.join(" AND ")} ORDER BY timestamp DESC`;
		const rows = this.db
			.query(sql)
			.all(...(values as SQLQueryBindings[])) as Record<string, unknown>[];
		return Promise.resolve(
			rows.map((r) => ({
				id: Number(r.id),
				session_id: String(r.session_id),
